...
func TestGetStatsForShortLink(t *testing.T) {
	// first create the shirt link
	rr := testCreateShortlink(t, cloudflare)
	...
	// now test the redirect
	req, err := http.NewRequest("get", "/{slug}/", nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	// now test the redirect
	req, err = http.NewRequest("get", "/link/{slug}/stats", nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	shortLinkHandler.GetShortLinkStats(rr, req)
